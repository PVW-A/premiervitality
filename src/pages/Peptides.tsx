...
const Peptides = () => {
  ...
  const goSignup = () => navigate("/auth?mode=signup&redirect=/peptides");
                              ...
                              return (
                                <div key={fKey}>
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      {isSingle ? (
                                        <span className="text-[10px] font-body text-muted-foreground/40">{form.variants[0].size} · {user ? `$${form.variants[0].price.toFixed(2)}` : <span onClick={(e) => { e.stopPropagation(); goSignup(); }} title="Create a free account to view pricing" className="inline-flex items-center gap-0.5 cursor-pointer hover:text-primary/60 transition-colors"><Lock size={8} />Members</span>}</span>
                                      ) : selVariant ? (
                                        <span className="text-xs font-body text-primary/60">{selVariant.size} · {user ? `$${selVariant.price.toFixed(2)}` : <span onClick={(e) => { e.stopPropagation(); goSignup(); }} title="Create a free account to view pricing" className="inline-flex items-center gap-0.5 cursor-pointer hover:text-primary/60 transition-colors"><Lock size={8} />Members</span>}</span>
                                      ) : (
                                      ...
                                      {form.variants.map(v => {
                                        ...
                                        return (
                                          <button key={v.size} onClick={() => setSelectedSizes(p => ({ ...p, [fKey]: v.size }))} className={`px-3 py-1.5 border text-[10px] font-body font-extralight transition-all ${isSel ? "border-primary/40 bg-primary/10 text-primary/70" : "border-border/30 text-muted-foreground/50 hover:border-border/50"}`}>
                                            {v.size}{isSel && (user ? <span className="ml-1.5 text-primary/60">${v.price.toFixed(2)}</span> : <span onClick={(e) => { e.stopPropagation(); goSignup(); }} title="Create a free account to view pricing" className="ml-1.5 text-primary/60 inline-flex items-center gap-0.5 cursor-pointer hover:text-primary transition-colors"><Lock size={8} />Members</span>)}
                                          </button>
                                        );
                                      })}
                                      {user && selVariant && (
