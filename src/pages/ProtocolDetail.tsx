...
import { ArrowLeft, FlaskConical, Clock, Tag, Layers } from "lucide-react";
import { openCalendly } from "@/hooks/useCalendly";
import {
  findProtocol,
  getProtocolDescription,
  getCompoundsFromName,
  tierMeta,
...
const ProtocolDetail = () => {
      ...
      >
        ...
        <main className="pt-32 pb-20 text-center px-6">
          ...
          <p className="text-muted-foreground font-body font-light text-sm mb-8 max-w-md mx-auto">
            The protocol you are looking for does not exist or may have been
            moved. Please browse our full catalog.
          </p>
          <Link
            to="/protocols"
            className="inline-flex items-center gap-2 text-primary hover:text-primary/80 font-body text-sm tracking-wide transition-colors"
          >
  ...
  const description = getProtocolDescription(card.name, category.name);
  const compounds = getCompoundsFromName(card.name);
      ...
      <main className="pt-24 pb-20">
        ...
        <div className="max-w-4xl mx-auto px-6 mb-10">
          ...
          >
            <Link
              to="/protocols"
              className="inline-flex items-center gap-2 text-muted-foreground/60 hover:text-primary font-body text-xs tracking-[0.15em] uppercase transition-colors"
            >
        ...
        <section className="max-w-4xl mx-auto px-6 mb-14">
          ...
          >
            {/* Tier badge */}
            <span
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-sm text-[9px] tracking-[0.25em] uppercase font-body font-light"
              style={{
                ...
                {compounds.map((compound, i) => (
                  ...
                  >
                    <div className="flex items-start gap-3 mb-3">
                      <div
                        className="mt-0.5 flex items-center justify-center w-7 h-7 rounded-sm shrink-0"
                        style={{
                          background: "rgba(171,143,95,0.08)",
                          border: "1px solid rgba(171,143,95,0.15)",
                        }}
                      >
                        <FlaskConical
                          size={13}
                      ...
                      <h3 className="text-base md:text-lg font-heading font-light text-foreground/90">
                        {compound.name}
                      </h3>
                    </div>
                    <p className="text-muted-foreground font-body font-light text-sm leading-relaxed mb-3">
